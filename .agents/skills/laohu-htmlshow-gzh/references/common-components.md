...
行内标记的含义只在这里定义：`**文字**` 是加粗，`==文字==` 是高亮，`<u>文字</u>` 或 `++文字++` 是下划线，`~~文字~~` 是删除线，反引号是行内代码。主题映射表只选择同一含义的视觉组件，可以改变配色、线条与容器，不能互换删除与强调。主题缺少对应组件时使用本文件的通用组件并适配主题色。荧光笔属于高亮的视觉变体，不接管删除线标记。

通用删除线（仅在所选主题没有删除线组件时使用）：

```html
<span style="color:#6B7280;text-decoration:line-through;"><span leaf="">删除文字</span></span>
```
...
<section style="margin:0 0 20px;border-radius:8px;overflow:hidden;background:#1E293B;box-shadow:0 4px 16px -8px rgba(15,23,42,0.4);">
  ...
  <section style="padding:11px 14px;">
    <p style="margin:0;white-space:pre-wrap;font-family:'SF Mono',Consolas,Monaco,monospace;font-size:13px;line-height:1.6;color:#E2E8F0;"><span leaf="">def make_skill(name):
    return f"已生成 {name}"
print(make_skill("gzh-design"))</span></p>
  </section>
</section>
```

代码、命令、配置和 Prompt 的原始字符必须保留，包括空格、制表符、空行及末尾换行；只做必要的 HTML 转义，不把缩进换成全角空格或不换行空格。整个代码文本放在一个 leaf 内，样式用 `white-space:pre-wrap` 保留空白并允许长行折行；代码内容在开始标签后立即插入，结束标签紧接最后一个源字符，不加入模板缩进或额外换行。行距与内边距由样式控制，不改内容补版式。上例展示真实四个普通空格的缩进；替换代码时连同完整文本替换，不逐行重新缩进。

交付前从生成的代码节点提取文字，与原代码逐字符比较；浏览器复制与公众号粘贴另行核验。平台若丢失空白，提供原始代码文件作为保真渠道并明确限制，不改变代码字符来伪装兼容，也不将静态通过宣称为平台通过。
...
<section style="margin:0 0 20px;border-radius:8px;overflow:hidden;background:#F6F8FA;border:1px solid #E5E7EB;border-left:3px solid #DC2626;">
  ...
  <section style="padding:11px 14px;">
    <p style="margin:0;white-space:pre-wrap;font-family:'SF Mono',Consolas,Monaco,monospace;font-size:13px;line-height:1.6;color:#24292F;"><span leaf="">npx skills add gzh-design</span></p>
  </section>
</section>
```

（左竖条 `#DC2626` 换成当前主题主色；多行及复制检查沿用 1a 的原始字符保留方法。）
