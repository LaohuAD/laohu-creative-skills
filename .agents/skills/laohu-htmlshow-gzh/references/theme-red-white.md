...
> 存在可导读的章节时按主入口决定内容。红底白字编号 + 深色标题；条目选择与数量遵循主入口“目录提取”；这里仅定义视觉与位置。
...
<section style="background:#FEF2F2;border-radius:10px;padding:18px 20px;margin-bottom:24px;border:1px solid #FECACA;">
  <p style="font-size:15px;color:#374151;margin:0;line-height:1.8;text-align:justify;">
    <span leaf="">{{引用内容，可含 7d 下划线等内联样式}}</span>
  </p>
</section>
```
...
<section style="display:flex;margin-bottom:24px;">
  ...
  <section style="flex:1;padding-bottom:12px;">
    ...
    <p style="font-size:15px;margin:0;color:#374151;line-height:1.8;text-align:justify;"><span leaf="">{{节点内容}}</span></p>
  </section>
</section>
```
...
多行代码块用通用增量库 `common-components.md` 的 1a 深色 / 1b 浅色（左竖条换 `#DC2626`），代码字符与显示方法沿用通用库。
...
| `## 章节标题` | 组件 5 章节标题 | 红底编号 01/02…，结语/总结末章用 ∞ + THE END，其他末章沿用数字 |
...
| ` ``` 多行代码块 ``` ` | 通用库 1a 深色 / 1b 浅色（左竖条换 #DC2626） | 原字符与空白按通用库 1a 保留 |
