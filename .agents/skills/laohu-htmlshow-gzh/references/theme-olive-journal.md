...
**用途**：文章开篇头图区，内刊风格：顶部标签+日期条，主标题+强调词，侧边配一枚菱形文字装饰，底部深色摘要条收尾。整篇文章唯一一处、放在最前面，不设 `margin-top`。
...
**用途**：技术流程/数据管线一类"从输入到输出"的示意图，用内联样式文本框保留输入、处理层和结果关系，节点名称与状态按实际流程替换，不嵌入 SVG。
...
<section style="margin-top:24px;">
  <section style="background:#eeefe9;border-radius:6px;padding:22px 18px;border:1px solid #bfc1b7;font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    ...
    <section style="padding:12px;background:#fdfdf8;border:1px solid #bfc1b7;border-radius:4px;"><p style="margin:0;color:#23251d;"><span leaf="">{{节点一}} → {{处理层：节点二、节点三、节点四、输出}} → {{结果}}</span></p><p style="margin:6px 0 0;color:#65675e;"><span leaf="">{{说明与结果状态}}</span></p></section>
