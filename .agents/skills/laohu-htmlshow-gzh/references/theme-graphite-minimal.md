...
> 存在可导读的章节时按主入口决定内容。上方极细线 + 极浅灰底，石墨灰编号 + 深炭标题，纯灰阶无橙色，大留白。条目选择与数量遵循主入口“目录提取”；这里仅定义视觉与位置。
...
<section style="background:#FAFAFA;border:1px solid #E4E4E7;padding:20px 22px;margin:0 10px 28px;">
  ...
  <p style="font-size:15px;color:#3F3F46;margin:0;line-height:1.8;text-align:justify;">
    <span leaf="">{{引用内容，可含 7d 石墨下划线等内联样式}}</span>
  </p>
</section>
```
...
<section style="display:flex;margin-bottom:24px;">
  ...
  <section style="flex:1;padding-bottom:12px;">
    ...
    <p style="font-size:15px;margin:0;color:#52525B;line-height:1.8;text-align:justify;"><span leaf="">{{节点内容}}</span></p>
  </section>
</section>
```
...
GIF 动图角标改极简描边胶囊：边框与字色用石墨主色 `#52525B`、背景透明。多行代码块用通用增量库 `common-components.md` 的 1a 深色 / 1b 浅色（左竖条换 `#52525B`），代码字符与显示方法沿用通用库。
...
| `## 章节标题` | 组件 5 超大水印编号 + 标题 | 自动编号 01/02…，结语/总结末章用 ∞ + THE END，其他末章沿用数字 |
...
| ` ``` 多行代码块 ``` ` | 通用库 1a 深色 / 1b 浅色（左竖条换 #52525B） | 原字符与空白按通用库 1a 保留 |
